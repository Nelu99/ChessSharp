...
public class ReginaNegru : MonoBehaviour {
	...
	void Awake()
	{
		...
		lvl2 = Resources.Load ("Bregina2", typeof(Sprite)) as Sprite;
	}
	...
	void Update () 
	{
		if (Input.GetMouseButton (0) == false && (clicked == true) && Matrice.turn == 2) 
		{
			...
			{
				var friends1 = GameObject.FindGameObjectsWithTag ("bpion");
				var friends2 = GameObject.FindGameObjectsWithTag ("btura");
				var friends3 = GameObject.FindGameObjectsWithTag ("bnebun");
				var friends4 = GameObject.FindGameObjectsWithTag ("bcal");
				foreach (var friend in friends1) 
					if(friend.transform.position == new Vector3 ((int)this.transform.position.x + 0.5f, (int)this.transform.position.y - 0.5f,0f))
					{
						...
						if (friend.GetComponent<PionNegru> ().lvl == 2)
						{
							Destroy(friend.GetComponent<PionNegru>());
							friend.AddComponent (typeof(PionNegru));
							friend.GetComponent<PionNegru> ().lvl = 2;
						} 
						else
						{
							Destroy(friend.GetComponent<PionNegru>());
				...
				foreach (var friend in friends2) 
					...
					{
						friend.transform.position = new Vector2 (y + 0.5f, (-1) * x - 0.5f);
						if (friend.GetComponent<TuraNegru> ().lvl == 2)
						{
							Destroy(friend.GetComponent<TuraNegru>());
							friend.AddComponent (typeof(TuraNegru));
							friend.GetComponent<TuraNegru> ().lvl = 2;
						} 
						else
						{
							Destroy(friend.GetComponent<TuraNegru>());
				...
				foreach (var friend in friends3) 
					...
					{
						friend.transform.position = new Vector2 (y + 0.5f, (-1) * x - 0.5f);
						if (friend.GetComponent<NebunNegru> ().lvl == 2)
						{
							Destroy(friend.GetComponent<NebunNegru>());
							friend.AddComponent (typeof(NebunNegru));
							friend.GetComponent<NebunNegru> ().lvl = 2;
						} 
						else
						{
							Destroy(friend.GetComponent<NebunNegru>());
				...
				foreach (var friend in friends4) 
					...
					{
						friend.transform.position = new Vector2 (y + 0.5f, (-1) * x - 0.5f);
						if (friend.GetComponent<CalNegru> ().lvl == 2)
						{
							Destroy(friend.GetComponent<CalNegru>());
							friend.AddComponent (typeof(CalNegru));
							friend.GetComponent<CalNegru> ().lvl = 2;
						} 
						else
						{
							Destroy(friend.GetComponent<CalNegru>());
		...
		if (y != 7)
			for (int i = y + 1; i <= 7; i++) {
				if (Matrice.board [x, i] == 0 && Matrice.moves [x, i] != (-1) && checkMove (x, i) == false)
					...
					return true;
				} else if (Matrice.board [x, i] != 0 && (Matrice.board [x, i] < 10 || Matrice.moves [x, i] == (-1) || Matrice.board [x, i] == 4 || Matrice.board [x, i] == 44))
					break;
			}
		if (y != 0)
			for (int i = y - 1; i >= 0; i--) {
		...
		if (x != 7)
			for (int i = x + 1; i <= 7; i++) {
				if (Matrice.board [i, y] == 0 && Matrice.moves [i, y] != (-1) && checkMove (i, y) == false)
					...
					return true;
				} else if (Matrice.board [i, y] != 0 && (Matrice.board [i, y] < 10 || Matrice.moves [i, y] == (-1) || Matrice.board [i, y] == 4 || Matrice.board [i, y] == 44))
					break;
			}
		if (x != 0)
			for (int i = x - 1; i >= 0; i--) {
			...
			if (y != 7)
				for (int i = y + 1; i <= 7; i++) {
					if (Matrice.board [x, i] == 0 && Matrice.moves [x, i] != (-1) && checkMove (x, i) == false && checkMove (x, i) == false)
						...
						Matrice.moves [x, i] = 1;
						break;
					} else if (Matrice.board [x, i] < 10 && Matrice.board [x, i] != 4 && Matrice.board[x,i]!=0) {
			...
			if (x != 7)
				for (int i = x + 1; i <= 7; i++) {
					if (Matrice.board [i, y] == 0 && Matrice.moves [i, y] != (-1) && checkMove (i, y) == false)
						...
						Matrice.moves [i, y] = 1;
						break;
					} else if (Matrice.board [i, y] < 10 && Matrice.board [i, y] != 4 && Matrice.board[i,y]!=0) {
			...
			while (m <= 7 && n >= 0) {
				...
				if (m <= 7 && n >= 0) {
					if (Matrice.board [m, n] == 0 && Matrice.moves [m, n] != (-1) && checkMove (m, n) == false)
						...
						Matrice.moves [m, n] = 1;
						break;
					} else if (Matrice.board [m, n] < 10 && Matrice.board [m, n] != 4 && Matrice.board[m,n]!=0) {
